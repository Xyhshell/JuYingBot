...
# 定时任务
from timeday.autowork import main_start
from timeday.ac_autowork import is_main
...
async def message_handler(recv, old_recv, handlebot):  # 处理收到的消息
    await asyncio.create_task(handlebot.message_handler(recv, old_recv))
                ...
                try:
                    ...
                    if r_type == 1 or r_type == 3 or r_type == 49:
                        ...
                        if isinstance(recv["content"], str):  # 判断是否为txt消息
                            old_recv = recv["content"]
                            asyncio.create_task(message_handler(recv, old_recv, handlebot)).add_done_callback(callback)
...
while not logged_in:
    try:
        ...
        time.sleep(3)
logger.info("已确认微信已登陆，开始启动Bot")

bot.start()  # 开启机器人

web_api_data = WebApiData()
handlebot = bybot.Bybot()

# 定时任务 TODO:定时
scheduler_thread = threading.Thread(target=main_start)
scheduler_thread.daemon = True
scheduler_thread.start()

# 异步定时任务 TODO:定时
acscheduler_thread = threading.Thread(target=is_main)
acscheduler_thread.daemon = True
acscheduler_thread.start()
